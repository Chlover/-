...
contract TimeDelayedVault {
    ...
    modifier onlyAuthorized() {
        ...
        // 检查是不是additionalAuthorizedContract
        if(additionalAuthorizedContract == msg.sender) {
            pass = true;
        }

        // 检查在不在authorizedUsers列表中
        for (uint i = 0; i < authorizedUsers.length; i++) {
    ...
    function withdrawFund() onlyAuthorized external returns (bool) {
        ...
        //delegatecall仅使用给定地址的代码，其它信息则使用当前合约(如存储，余额等等)。
        //此处为调用调用另一个合约（CommonWalletLibrary）basicWithdraw函数
    ...
    // 用于检查是否所有人都vote过了，所有人都vote过才返回true
    ...
    function addAuthorizedAccount(uint votePosition, address proposal) onlyAuthorized external {
        //votePosition不能乱填 只能填0~5（我们组一共6个人）
        require(votePosition < authorizedUsers.length);
        //要对应好自己的votePosition与地址
        require(msg.sender == authorizedUsers[votePosition]);

        //proposalAAA应该是第一个人输入的时候的地址，应该是以后每个人调用的时候都要输入这个地址，这样才不会重新vote
        //最后就会把proposalAAA赋给additionalAuthorizedContract
        ...
        //注意！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
        //只有所有人都vote过了才会给additionalAuthorizedContract赋值
        if (checkAllVote()) {
